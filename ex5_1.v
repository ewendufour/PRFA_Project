...
Inductive form : Type :=
| var ( x : nat) | bot | imp ( s t : form) | and (s t : form).
Print In.
Print incl.
...
Inductive ndc : list form -> form -> Prop:=
  ...
  | CAndI A s t: ndc A s -> ndc A t -> ndc A (and s t)
  | CAndE1 A s t: ndc A (and s t) -> ndc A s  
  | CAndE2 A s t: ndc A (and s t) -> ndc A t.
...
Inductive ground : form -> Prop:=
  ...
  | Gand s t: ground s  -> ground t  -> ground (and s t).
